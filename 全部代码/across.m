function NewChrom=across(Chrom,XOVR,PNumber,JmNumber)
...
for i=1:2:Num
    if XOVR>rand; 
        ...
        for kk=ii+1:WNumber
            if NewChrom(i,ii)==NewChrom(i,kk)
                while NewChrom(i,ii+WNumber)==NewChrom(i,kk+WNumber)
                    NewChrom(i,kk+WNumber)=unidrnd(JmNumber);
                end
            end
        end
        end
        ...
        for ii=1:WNumber-1   %杜绝同一机器加工两次
        for kk=ii+1:WNumber
            if NewChrom(i+1,ii)==NewChrom(i+1,kk)
                while NewChrom(i+1,ii+WNumber)==NewChrom(i+1,kk+WNumber)
                    NewChrom(i+1,kk+WNumber)=unidrnd(JmNumber);
                end
            end
        end
        end
